...
### 나. 핵심 어노테이션: `@MockitoUnitTest`와 `@SpringUnitTest`

프로젝트에서는 단위 테스트의 목적에 따라 두 가지 커스텀 어노테이션을 제공합니다.

- **`@MockitoUnitTest`**: 스프링 컨텍스트 없이, 순수 Mockito 기능만으로 테스트할 때 사용합니다.
  - `@ExtendWith(MockitoExtension.class)`를 포함하여 `@Mock`, `@InjectMocks` 등을 사용할 수 있습니다.

- **`@SpringUnitTest`**: Mockito 기능과 함께 테스트에 필요한 최소한의 스프링 설정(e.g., `@Import`)을 가져와야 할 때 사용합니다.
  - 예시) `@Import(FixedClockConfig.class)`를 통해 고정된 시간을 주입받는 테스트

### 다. 작성 예시: `JwtAuthenticatorTest.java`

`JwtAuthenticator`는 토큰 문자열을 받아 그 유효성을 검증하고 인증 객체를 만드는 순수한 책임을 가집니다. 외부 의존성인 `JwtTokenPort`는 `@Mock`으로 가짜 객체를 만들어 `JwtAuthenticator`의 로직에만 집중합니다. 이 예시에서는 `@SpringUnitTest`를 사용했지만, 만약 `FixedClockConfig` 같은 `@Import`가 필요 없다면 `@MockitoUnitTest`를 사용하는 것이 더 적합합니다.
